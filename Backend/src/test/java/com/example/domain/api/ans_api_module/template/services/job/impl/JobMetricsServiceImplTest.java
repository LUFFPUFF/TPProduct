...
// import com.example.domain.api.ans_api_module.template.services.job.JobMetricsService; // Не нужен для теста реализации
...
import org.springframework.batch.core.*; // Импортируем все из core
import org.springframework.batch.item.ExecutionContext; // Отдельный импорт для ExecutionContext
import org.springframework.dao.DataAccessException; // Пример ошибки от коллектора
...
class JobMetricsServiceImplTest {
    ...
    void setUp() {
        ...
        mockJobParameters = new JobParameters(); // Можно использовать реальный или мок
        mockExecutionContext = new ExecutionContext(); // Можно использовать реальный или мок

        // Создаем мок JobExecution, который будет возвращаться маппером
        // JobInstance нужен для конструктора JobExecution
