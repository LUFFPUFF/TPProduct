...
class RegistrationServiceImplTest {
    ...
    @Test
    void checkRegistrationCode_InvalidCode_ShouldThrowInvalidRegistrationCodeException() {
        // Arrange
        when(authCacheService.getRegistrationCode(REGISTRATION_CODE)).thenReturn(Optional.empty());
        ...
        verify(authCacheService).getRegistrationCode(REGISTRATION_CODE);
        verifyNoInteractions(mapperDto, userRepository, userDetailsService, jwtUtilsService);
    }
    ...
    // Приватный метод generateRegistrationCode не тестируем напрямую юнит-тестами,
    // так как он приватный и его логика (генерация и проверка уникальности)
    // должна быть проверена через публичные методы, которые его используют (если бы он использовался).
    // В данном случае он не используется публичным API, так как sendRegistrationCode хардкодит "000000".
}